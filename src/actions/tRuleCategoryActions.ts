import API from "../api/TRuleType";
import snackBarUpdate from "./snackBarActions";
import { updateModal } from "./modalActions";
import { ACTIONS } from '../interfaces/actionTypes/tRuleTypes';
...
export const create = (body: object) => async (dispatch: Function) => {
  ...
  try {
    ...
    if (status === 200 || status === 201) {
      ...
      snackBarUpdate({
        payload: {
          message: "Rule ha sido Registrado!",
          type: "success",
          status: true
        }
      })(dispatch);
...
export const update = (body: object) => async (dispatch: Function) => {
  ...
  try {
    ...
    if (status === 200) {
      ...
      snackBarUpdate({
        payload: {
          message: "Rule ha sido Actualizado!",
          type: "success",
          status: true
        }
      })(dispatch);
...
export const remove = (id: number) => async (dispatch: Function) => {
  try {
    ...
    if (status === 200) {
      ...
      snackBarUpdate({
        payload: {
          message: "Rule ha sido Elmininado!",
          type: "success",
          status: true
        }
      })(dispatch);
